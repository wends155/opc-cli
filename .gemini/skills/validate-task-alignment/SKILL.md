...
5. **Objective Check:** Extract all rows from the `### Plan Objectives` table in the plan (by ID). Verify each objective ID appears in the `## Plan Objectives` table in `task.md` with matching Objective and Success Criteria text. Report missing or mismatched rows.
6. **Forward Check:** Verify every extracted file entry from the plan acts as a checklist item in `task.md`.
7. **Reverse Check:** Verify every file entry in `task.md` corresponds directly to a stated plan entry.
8. Report the alignment results:
   - If perfectly aligned: `"✅ task.md aligned with plan (objectives + implementation steps)"`
