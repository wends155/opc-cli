...
# See .agents/workflows/update-doc.md, architecture.md for the current inline procedures.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Scan-ProjectDocs.ps1 -Mode scan

.EXAMPLE
    .\.agents\scripts\Scan-ProjectDocs.ps1 -Mode validate
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
function Add-ValidationResult {
    ...
    if (-not $Present) {
        $script:ValidationFailures += "$DocFile S $Section"
    }
}
...
if ($TreeCmd) {
    ...
    Get-ChildItem -Path $RepoRoot -Recurse -Depth 3 -Name -ErrorAction SilentlyContinue |
        Where-Object { $_ -notmatch '(\.git|node_modules|target|\.agents)' } |
...
if ($Scope -in @('arch', 'all')) {
    ...
    if (Test-Path $ArchFile) {
        ...
        foreach ($section in $ArchSections) {
            ...
            $Status = if ($Found) { "[OK] Present" } else { "[FAIL] Missing" }
        ...
        if ($SpecRaw -match '(?m)^>\s*Last verified against:\s*(\S+)') {
            ...
            if ($LASTEXITCODE -eq 0 -and $CommitsBehind) {
                ...
                if ($CommitsBehind -eq 0) {
                    Write-Output "[OK] spec.md is current (verified against ``$VerifiedHash``, 0 source commits since)"
                } else {
                    Write-Output "[WARN] spec.md may be stale (**$CommitsBehind source commit(s)** since last verification at ``$VerifiedHash``)"
                    if ($Mode -eq 'validate') {
                        Write-Output "> Run ``/update-doc`` to re-verify and update the hash."
                    }
                }
            } else {
                Write-Output "[WARN] Could not resolve hash ``$VerifiedHash`` - git history may be shallow or hash is invalid."
            }
        } else {
            Write-Output "[WARN] No ``Last verified against:`` metadata found in spec.md."
...
if ($Mode -eq 'validate') {
    ...
    if ($script:ValidationFailures.Count -eq 0) {
        Write-Output "[OK] **All required sections are present.**"
        exit 0
    }
    else {
        Write-Output "[FAIL] **$($script:ValidationFailures.Count) required section(s) missing:**"
        Write-Output ""
