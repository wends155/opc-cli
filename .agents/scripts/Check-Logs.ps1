...
# See .agents/workflows/log-audit-generic.md for the current inline procedure.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Check-Logs.ps1 -Level TRACE

.EXAMPLE
    .\.agents\scripts\Check-Logs.ps1 -LogDir "target/logs" -Level DEBUG -Lifecycle

.EXAMPLE
    .\.agents\scripts\Check-Logs.ps1 -Level TRACE -DeepAnalysis
#>
[CmdletBinding()]
param(
    [string]$LogDir = 'logs',
...
if (-not (Test-Path $LogDir)) {
    ...
    Write-Output ""
    Write-Output "> [FAIL] **Log directory not found:** ``$LogDir``"
...
if ($logFiles.Count -eq 0) {
    Write-Output "# Log Audit"
    Write-Output ""
    Write-Output "> [FAIL] **No log files found in** ``$LogDir``"
    exit 1
}
...
if ($DeepAnalysis) {
    ...
    Write-Output "### SB. Connection Churn"
    Write-Output ""
    Write-Output "| Metric | Value |"
    Write-Output "|--------|-------|"
    ...
    # --- SC. Repetition Analysis ---
    Write-Output "### SC. Top Repeated Messages"
    Write-Output ""
    $grouped = $deepContent |
    ...
    # --- SD. Span Integrity ---
    Write-Output "### SD. Span Integrity"
    Write-Output ""
...
if ($hasErrors) {
    Write-Output "> [FAIL] **Errors detected.** Use the /log-audit workflow for deep analysis."
} else {
