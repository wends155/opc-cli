...
# See .agents/workflows/toolcheck.md for the current inline procedure.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Check-Environment.ps1 -Mode scan
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
Write-Output "# [TOOL] Environment Scan"
...
Write-Output "| pwsh | [OK] | $PwshVersion ($PwshEdition) | (current shell) |"

foreach ($tool in $ShellTools) {
    $result = Test-Tool -Name $tool.Name -VersionCmd $tool.VersionCmd
    if ($result.Found) {
        Write-Output "| $($tool.Name) | [OK] | $($result.Version) | ``$($result.Path)`` |"
    } else {
        Write-Output "| $($tool.Name) | [FAIL] | - | not found |"
    }
}
...
if ($GpgSign -eq 'true') {
    Write-Output "> [WARN] **GPG signing enabled.** Ensure gpg-agent is running or commits may hang."
}
...
if ($FixesApplied.Count -gt 0) {
    ...
    Write-Output ""
    Write-Output "> [OK] Non-interactive env vars set **for this session**."
} else {
    Write-Output "> [OK] No interactive blockers detected."
}
...
foreach ($tool in $RustTools) {
    $result = Test-Tool -Name $tool.Name -VersionCmd $tool.VersionCmd
    if ($result.Found) {
        Write-Output "| $($tool.Name) | [OK] | $($result.Version) |"
        if ($tool.Name -eq 'rustc') { $RustAvailable = $true }
    } else {
        Write-Output "| $($tool.Name) | [FAIL] | - |"
    }
}
    ...
    if ($cmd) {
        ...
        if ($linker -eq 'link.exe') {
            if ($path -match '(scoop|shims|busybox|coreutils|usr\\bin)') {
                $notes = "[WARN] **CONFLICT**: This is scoop/coreutils ``link``, NOT MSVC ``link.exe``"
            } elseif ($path -match '(MSVC|Microsoft|VC|Visual)') {
                $notes = "[OK] MSVC linker"
            } else {
                $notes = "[WARN] Unknown origin - verify this is MSVC"
            }
        }

        Write-Output "| $linker | [OK] | ``$path`` | $notes |"
    } else {
        Write-Output "| $linker | [FAIL] | not found | - |"
    }
}
...
if ($AllLinks.Count -gt 1) {
    Write-Output ""
    Write-Output "> [WARN] **Multiple ``link.exe`` found on PATH** ($($AllLinks.Count) instances):"
...
$WorkflowDir = Join-Path (Join-Path $RepoRoot ".agents") "workflows"
...
foreach ($group in $AlternativeWorkflows.GetEnumerator()) {
    ...
    if ($found) {
        Write-Output "| $found | [OK] |"
    } else {
        $names = ($group.Value -join ' or ')
        Write-Output "| $names | [FAIL] |"
        $WorkflowMissing++
    }
}
...
$ScriptDir = Join-Path (Join-Path $RepoRoot ".agents") "scripts"
$ExpectedScripts = @(
...
Write-Output "| **Rust** | $( if ($RustAvailable) { '[OK]' } else { '[FAIL]' } ) |"

Write-Output ""
Write-Output "> **Narsil Best Practice:** Set ``path=""$RepoRoot""`` in Narsil tool calls to restrict analysis to this specific project."
Write-Output "> Scan complete. LLM should now diagnose any [FAIL] items and attempt fixes."
