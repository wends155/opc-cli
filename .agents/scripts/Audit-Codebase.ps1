...
#>
<#
# [WARN] REFERENCE ONLY - Workflows now use inline auto-runnable commands.
# This script is retained as documentation of the procedure it implements.
# See .agents/workflows/audit.md for the current inline procedure.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Audit-Codebase.ps1 -Mode scan

.EXAMPLE
    .\.agents\scripts\Audit-Codebase.ps1 -Mode scan -Scope full

.EXAMPLE
    .\.agents\scripts\Audit-Codebase.ps1 -Mode gate
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
function Invoke-GateCheck {
    ...
    if (-not $Command) {
        Write-Output "| **$Name** | ``N/A`` | [SKIP] Skipped (no command) |"
        return $true
    }
    ...
    Write-Output "| **$Name** | ``$Command`` | $(
        try {
            ...
            if ($LASTEXITCODE -eq 0) { "[OK] Pass" } else { "[FAIL] Fail" }
        }
        catch {
...
if ($FmtCmd) {
    ...
    Write-Output "| **Formatter** | N/A | [SKIP] Skipped |"
}
...
if ($LintCmd) {
    ...
    Write-Output "| **Linter** | N/A | [SKIP] Skipped |"
}
...
if ($TestCmd) {
    ...
    Write-Output "| **Tests** | N/A | [SKIP] Skipped |"
}

Write-Output ""
if ($GatePass) {
    Write-Output "[OK] **Verification gate passed.**"
}
else {
    Write-Output "[FAIL] **Verification gate failed.** See details above."
}
...
if ($Language -eq "Rust") {
    ...
    if ($UnwrapHits) {
        Write-Output "[FAIL] **Found $(@($UnwrapHits).Count) instance(s):**"
        Write-Output ""
        Write-Output '```'
        ...
        Write-Output "[OK] No ``.unwrap()`` found in production code."
    }
}
...
if ($SecretHits) {
    Write-Output "[WARN] **Found $(@($SecretHits).Count) potential secret(s):**"
    Write-Output ""
    Write-Output '```'
    ...
    Write-Output "[OK] No hardcoded secret patterns detected."
}
...
if ($TodoHits) {
    ...
    Write-Output "[OK] No TODO/FIXME/HACK markers found."
}
...
Write-Output ""
Write-Output "Items marked with [AUTO] were checked automatically. Items marked with [MANUAL] require LLM reasoning."
Write-Output ""

# Gate results
$GateStatus = if ($GatePass) { "[OK]" } else { "[FAIL]" }
Write-Output "### Verification Gate"
Write-Output "- [$GateStatus] [AUTO] Formatter, Linter, Tests pass"
Write-Output ""

# Mechanical results
$UnwrapStatus = if ($Language -ne "Rust") { "N/A" } elseif (-not $UnwrapHits) { "[OK]" } else { "[FAIL]" }
$SecretStatus = if (-not $SecretHits) { "[OK]" } else { "[WARN]" }

Write-Output "### Code Quality (2e)"
Write-Output "- [$UnwrapStatus] [AUTO] No ``.unwrap()`` in production code"
Write-Output "- [$SecretStatus] [AUTO] No hardcoded secrets"
Write-Output "- [ ] [MANUAL] Code is idiomatic"
Write-Output "- [ ] [MANUAL] No dead code or unused imports"
Write-Output "- [ ] [MANUAL] Clear variable/function names"
Write-Output ""

Write-Output "### GEMINI.md Compliance (2b)"
Write-Output "- [ ] [MANUAL] Error handling: no silent failures"
Write-Output "- [ ] [MANUAL] Observability: structured logging present"
Write-Output "- [ ] [MANUAL] Documentation: public items documented"
Write-Output ""

Write-Output "### Testing (2c)"
Write-Output "- [ ] [MANUAL] Tests exist for new/changed logic"
Write-Output "- [ ] [MANUAL] Edge cases covered"
Write-Output "- [ ] [MANUAL] Testable design (injectable deps)"
Write-Output ""
...
Write-Output "> Items marked [MANUAL] require the Architect's judgment (or Narsil MCP)."
