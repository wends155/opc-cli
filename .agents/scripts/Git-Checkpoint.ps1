...
# See .agents/workflows/build.md for the current inline procedure.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Git-Checkpoint.ps1 -Files "src/main.rs","Cargo.toml" -Message "feat: add parser"

.EXAMPLE
    .\.agents\scripts\Git-Checkpoint.ps1 -Files "." -Message "fix: resolve lint" -ValidateTask
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
if ($StatusOutput) {
    ...
    Write-Output '```'
    Write-Output ""
    Write-Output "> [WARN] Nothing to commit. Working tree is clean."
    exit 0
}
Write-Output '```'
Write-Output ""
...
foreach ($f in $Files) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Output "[FAIL] ``git add $f`` failed: $addOutput"
        exit 3
    }
    Write-Output "[OK] Staged: ``$f``"
}
...
if ($ValidateTask) {
    ...
    $ScriptPath = Join-Path (Join-Path (Join-Path $RepoRoot ".agents") "scripts") "Sync-TaskList.ps1"
    if (-not (Test-Path $ScriptPath)) {
        Write-Output "[WARN] Sync-TaskList.ps1 not found - skipping validation."
    }
    elseif (-not $PlanFile) {
        Write-Output "[WARN] ``-PlanFile`` not provided - skipping task validation."
        ...
        $validateOutput = & $shellCmd -NonInteractive -File $ScriptPath @validateArgs 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Output "[FAIL] **task.md validation failed.** Aborting commit."
            Write-Output ""
            Write-Output '```'
            $validateOutput
            Write-Output '```'
        ...
        Write-Output "[OK] task.md validation passed."
        # Show advisory output (progress warnings etc.)
        $advisoryLines = $validateOutput | Where-Object { $_ -match '[WARN]|still unchecked|Update task' }
        if ($advisoryLines) {
...
if ($LASTEXITCODE -ne 0) {
    Write-Output "[FAIL] ``git commit`` failed:"
    Write-Output ""
    Write-Output '```'
    $commitOutput
    Write-Output '```'
