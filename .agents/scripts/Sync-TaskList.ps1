...
# See .agents/workflows/plan-making.md, build.md for the current inline procedures.
.SYNOPSIS
...
.EXAMPLE
    .\.agents\scripts\Sync-TaskList.ps1 -Mode generate -PlanFile plan.md

.EXAMPLE
    .\.agents\scripts\Sync-TaskList.ps1 -Mode validate -PlanFile plan.md -TaskFile task.md

.EXAMPLE
    .\.agents\scripts\Sync-TaskList.ps1 -Mode doc-list -PlanFile plan.md
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
if ($Mode -eq 'generate') {
    ...
    Write-Output "[OK] task.md written to: $TaskFile ($($FileEntries.Count) file entries)"
...
if ($Mode -eq 'validate') {
    ...
    if ($StaleItems.Count -gt 0) {
        Write-Output ""
        Write-Output "[WARN] **$($StaleItems.Count) item(s) still unchecked:**"
        Write-Output ""
...
if ($Mode -eq 'preflight') {
    ...
    # --- Report ---
    if ($Errors.Count -eq 0) {
        Write-Output "[OK] **Pre-flight passed.** task.md exists, has items, and aligns with plan."
        Write-Output "> Files: $($FileEntries.Count) | Task: $TaskFile"
        exit 0
    }
    else {
        Write-Output "[FAIL] **Pre-flight FAILED** ($($Errors.Count) issue(s)):"
        Write-Output ""
        foreach ($e in $Errors) {
            Write-Output "- $e"
        }
...
if ($Mode -eq 'doc-list') {
    ...
    if ($NewFiles) {
        Write-Output "### [NEW] NEW - Needs fresh documentation"
    ...
    if ($ModFiles) {
        Write-Output "### [MODIFY] MODIFY - Check for doc drift"
    ...
    if ($DelFiles) {
        Write-Output "### [DELETE] DELETE - Clean up references"
    ...
    Write-Output "3. Prioritize [NEW] NEW files (no docs exist yet)."
    Write-Output "4. Check [MODIFY] MODIFY files for changed interfaces or behavior."
    Write-Output "5. Clean up [DELETE] DELETE references from ``architecture.md`` and ``spec.md``."
