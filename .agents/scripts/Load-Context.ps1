...
# See .agents/workflows/issue.md, feature.md, plan-making.md, toolcheck.md for the current inline procedures.
.SYNOPSIS
...
.DESCRIPTION
    ...
    Modes:
      ...
      plan  - All of the above + .agents/rules/coding-standard.md, .agents/rules/ipr.md, plan header template.

.PARAMETER Mode
    The workflow mode: 'issue' or 'plan'.

.EXAMPLE
    .\.agents\scripts\Load-Context.ps1 -Mode issue

.EXAMPLE
    .\.agents\scripts\Load-Context.ps1 -Mode plan
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
# --- Plan-only extras ---
if ($Mode -eq 'plan') {
    Show-DocIfExists '.agents/rules/coding-standard.md'
    Show-DocIfExists '.agents/rules/ipr.md'
}
