...
2. Extract the `### Plan Objectives` table rows from the plan (ID, Objective, Success Criteria, Steps columns).
3. Extract all headings matching `### ComponentName` and `#### [NEW|MODIFY|DELETE|TEST] filename`.
4. Write `task.md` to the same directory as the plan:
...
> [!NOTE]
> The task.md Plan Objectives table replaces the Steps column with a `✅` (checkbox) column for Builder progress tracking. The Steps column is only in the plan — the Builder doesn't need it in task.md.
...
Before requesting approval, verify these 7 critical fail-path checks:
...
7. **🧠 Plan Objectives check** — Verify that `### Plan Objectives` table is present in the plan, contains at least one data row with non-empty Objective and Success Criteria columns, and each row is mapped to the `## Plan Objectives` table in `task.md`.

> [!CAUTION]
> All 7 checks MUST pass before requesting approval. If any fails, fix and re-check.
