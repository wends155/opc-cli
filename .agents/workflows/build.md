...
5. Read the approved plan's **Plan Objectives** section — understand the high-level success criteria and what "done" looks like before starting execution.
6. Read the approved plan's **Negative Scope** section — files and areas NOT to touch.
7. Read `architecture.md` (if present) for project-specific design and toolchain commands.
8. If this is a multi-phase plan, read the prior phase's Phase Manifest (Deferred table in `context.md` or the prior plan). Verify that any `STUB(Phase N)` items where N = current phase are addressed as plan steps.
9. Confirm you are operating as the **Builder** (fast/efficient model).
