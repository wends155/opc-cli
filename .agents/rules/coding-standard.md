...
Every PR / commit **must** pass all 8 gates before merge:
...
cargo test --doc --workspace                                        # Gate 3: Doc Compilation Check
cargo test --workspace                                              # Gate 4: Unit & Integration Tests
cargo build --manifest-path compat/<crate>/Cargo.toml --release     # Gate 5: Polyfill Compilation
sg scan                                                             # Gate 6: AST Linting (conditional)
rg --color=never -n -g "*.rs" "\b(println!|dbg!|todo!)" opc-da-client/src/  # Gate 7: Forbidden Pattern Scanner
pwsh -Command "[System.Management.Automation.Language.Parser]::ParseFile(...)" # Gate 8: PowerShell Script Syntax Check
```
...
| Forbidden Patterns | Zero forbidden macros (`println!`, `dbg!`, `todo!`) in library code |
| Script Integrity | 100% PowerShell script syntax validation |
