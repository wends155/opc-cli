...
*   **Failure Cooldown Circuit Breaker:** Unresponsive remote endpoints trigger a 5-second cooldown in `failure_cooldowns` bounded to `MAX_COOLDOWNS = 256` with LRU eviction and expired entry pruning to avoid connection storm panics and unbounded memory growth. Configuration errors (such as invalid ProgID `CO_E_CLASSSTRING`) do not engage the connection cooldown.
*   **Array Length Parity Invariant:** `populate_item_states` and `handle_write_batch` strictly assert that server-returned results match requested valid handle lengths, failing fast with `OpcError::Internal` rather than silently truncating or misaligning results.
*   **Fault-Tolerant Recursive Browsing:** Individual leaf item failures in `browse_recursive` log structured errors via `log_opc_err!` without aborting traversal, ensuring valid sibling leaves are collected.
