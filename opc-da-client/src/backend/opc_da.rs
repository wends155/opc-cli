    ...
    async fn list_servers(&self, host: &str) -> Result<Vec<String>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for guid in guid_iter.flatten() {
                // SAFETY: `opc_da::GUID` and `windows::core::GUID` are binary
                // compatible 128-bit structures with identical field layouts
                // (4-2-2-8 byte segments). `transmute_copy` is used because
                // the crates define distinct types with the same ABI.
            ...
            servers.sort();
            servers.dedup();
            tracing::debug!(count = servers.len(), "Server enumeration complete");
            Ok(servers)
        })
        .await?
    }
