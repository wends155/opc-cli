...
- [ ] **Explicit Windows Authentication / Credentials Support**: Add CLI arguments and dialog options in `opc-cli` and `opc-da-client` (`COAUTHIDENTITY` configuration) to allow authenticating as a specific domain or local Windows user when connecting to remote DCOM hosts, rather than relying solely on the current process security token.
